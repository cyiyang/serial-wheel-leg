function K = series_get_k1_length(leg_length,ak)
   
    syms x(t) T R Iw mw L theta(t) mp g Ip 
    syms f1 f2 d_theta d_x theta0 x0  

    lw=polyval(ak,leg_length);        %摆杆质心高度（以轮轴为原点）
    lb=leg_length+0.13;               %机体质心高度
    R1=0.08;                           %驱动轮半径
    mw1=1.18;                         %驱动轮质量
    mb=10;                            %机体质量
    ml=0.54;                          %摆杆质量
    mp1=mb+ml;                        %杆质量+机体
    L1=(lb*10+lw*0.54)/mp1;           %总重心到驱动轮轴距离
    ...
    Q=diag([600 1 1000 100]);%theta d_theta x d_x %700 1 600 200 
    R=100;                %T 
