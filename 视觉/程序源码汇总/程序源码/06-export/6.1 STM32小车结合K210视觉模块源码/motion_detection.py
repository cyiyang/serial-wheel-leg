...
def on_timer(timer):
    global car_count, car_state, motion_state
    if motion_state > 0:
        if car_state:
            car_state = 0
            RGB.set(0, 0, 0)
...
def state_classify(self, state, event):
    ...
    if event == EVENT.BOOT_KEY:
        central_msg = None
        bot.set_car_motion(car_speed, 0, 0)
    if event == EVENT.BOOT_KEY_LONG_PRESS:
        restart(self)
        return
...
def control_led(class_id, high):
    global bottom_msg, motion_state
    if class_id == 1:#
        bottom_msg = "close,score:{:2.1f}".format(high)
        motion_state = 0
        RGB.set(0, 255, 0)
    elif class_id == 2:#
        motion_state = 1
        bottom_msg = "open,score:{:2.1f}".format(high)
        #RGB.set(255, 0, 0)
    elif class_id == 3:#
        motion_state = 1
        bottom_msg = "open,score:{:2.1f}".format(high)
...
def loop_classify():
    ...
    if high > THRESHOLD:
        class_id = index + 1
        #bottom_msg = "class:{},score:{:2.1f}".format(class_id, high)
        control_led(class_id, high)
    else:
        bottom_msg = None
