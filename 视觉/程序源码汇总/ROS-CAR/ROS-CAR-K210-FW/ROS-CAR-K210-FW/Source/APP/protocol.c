...
void Upper_Data_Parse(uint8_t *data_buf, uint8_t num)
{
	...
	case FUNC_MOTOR:
	{
		...
		speed[0] = (int16_t)motor_1 * ((MOTOR_MAX_PULSE-MOTOR_IGNORE_PULSE) / 100.0);
		speed[1] = (int16_t)motor_2 * ((MOTOR_MAX_PULSE-MOTOR_IGNORE_PULSE) / 100.0);
		speed[2] = (int16_t)motor_3 * ((MOTOR_MAX_PULSE-MOTOR_IGNORE_PULSE) / 100.0);
		speed[3] = (int16_t)motor_4 * ((MOTOR_MAX_PULSE-MOTOR_IGNORE_PULSE) / 100.0);
		// PWM控制小车运动
