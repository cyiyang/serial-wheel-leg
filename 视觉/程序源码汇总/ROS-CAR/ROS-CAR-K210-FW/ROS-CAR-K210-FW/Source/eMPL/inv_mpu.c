...
#define MPU9250                     //定义我们使用的传感器为MPU9250
...
// dmp初始化, 返回值:0,正常, 其他,失败
u8 mpu_dmp_init(uint8_t orient)
{
    u8 res = 0;
    if (mpu_init() == 0) //初始化MPU
    {
        ...
        {
            res = dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation_original)); //设置陀螺仪方向
        }
        if (res) return 5;  
        //设置dmp功能
...
// 得到dmp处理后的数据
u8 mpu_dmp_get_data(mpu_data_t *mpu_data)
{
